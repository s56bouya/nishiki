...
if( has_post_thumbnail() ){
	if( ( is_single() && get_theme_mod( 'setting_post_eye_catch', false ) == true ) or ( is_page() && get_theme_mod( 'setting_page_eye_catch', false ) == true ) ){
		...
		$class = ' style="background-image:url(' . esc_url_raw( $image ) . ');"';
	}
} else {
	$class = '';
}
?>
<header<?php echo $class; ?> class="<?php echo esc_attr( get_post_type() ); ?>">
	<div class="mask"></div>
	<div class="page_header container">
		<?php the_title( '<h1>', '</h1>' ); ?>
		<?php if( is_single() && !is_attachment() ){ ?>
		<div class="date">
			<time datetime="<?php echo esc_attr( get_the_time('Y-m-d') ); ?>"><?php esc_html_e( 'published', 'nishiki' ); ?>:<?php the_time('Y/m/d') ?></time>
			<?php if( get_the_time('Y-m-d') < get_the_modified_time('Y-m-d') ){ ?>
				<time datetime="<?php echo esc_attr( get_the_modified_time('c') ); ?>"><?php esc_html_e( 'edited', 'nishiki' ); ?>:<?php the_modified_date('Y/m/d') ?></time>
			<?php } ?>
		</div>
		<?php
