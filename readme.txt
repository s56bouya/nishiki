=== Nishiki ===
Requires at least: WordPress 4.5
Tested up to: WordPress 5.2.0
Version: 2.0.7
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: one-column, two-columns, right-sidebar, left-sidebar, flexible-header, accessibility-ready, custom-colors, custom-header, custom-menu, custom-logo, featured-images, footer-widgets, post-formats, sticky-post, theme-options, threaded-comments, translation-ready, blog, entertainment, portfolio, editor-style, block-styles, wide-blocks, footer-widgets, full-width-template

== Description ==

Nishiki is a fully responsive theme. Elegance,Refined,Multifunctional.

* Responsive layout.
* Front Page Template.
* Play video with Main Visual(YouTube or mp4).

In your admin panel, go to Appearance -> Customize.
About 80 customization can be done without writing the code.
Gutenberg Block Editor Style Support.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Nishiki in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Copyright ==

Nishiki WordPress Theme, Copyright 2018 AnimaGate, Inc.
Nishiki is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

All the photos used in the screenshot.png taken by myself, copyright 2017 s56bouya, distributed under the term of GPL.
carp.jpg and sky.jpg is taken by myself, copyright 2017 s56bouya, distributed under the term of GPL.

Nishiki WordPress Theme bundles the following third-party resources:

IcoMoon License
License - GPL / CC BY 4.0
URL: https://icomoon.io/#icons-icomoon
Created by Keyamoon

== Changelog ==

= 2.0.7 =
* Change : header image size

= 2.0.6 =
* Change : Several minor changes
* Add : Support WP 5.2.0

= 2.0.5 =
* Change : Several minor changes

= 2.0.4 =
* Add : Translate

= 2.0.3 =
* Add : Support WP 5.0.3
* Add : Icon font
* Change : Several minor changes

= 2.0.2 =
* Add : Support WP 5.0.2
* Change : Several minor changes

= 2.0.1 =
* Add : Readme text(Gutenberg Block Editor Support)
* Change : Several minor changes

= 2.0.0 =
* Change : Don't smooth scroll at WooCommerce image zoomup
* Add : Adjust alignwide size and alignfull size when displaying sidebar-left, sidebar-right

= 1.0.40 =
* Fixed : Don't smooth scroll at WooCommerce tabs and stars

= 1.0.39 =
* Delete : embed_oembed_html filter and css style

= 1.0.38 =
* Change : Gutenberg css style(alignfull,alignwide)
* Change : Several minor changes

= 1.0.37 =
* Change : Gutenberg css style

= 1.0.36 =
* Add : Support WP 5.0
* Add : Related posts

= 1.0.35 =
* Add : Customizer header contents width
* Add : Customizer archive contents width
* Add : Customizer footer contents width

= 1.0.34 =
* Add : Customizer archive page sidebar

= 1.0.33 =
* Fixed : Page for posts header background attachment style

= 1.0.32 =
* Add : Header title of post page when front-page.php is set

= 1.0.31 =
* Add : Polyfill(object-fit-images)

= 1.0.30 =
* Fixed : Front section image load JavaScript

= 1.0.29 =
* Change : Front section style

= 1.0.28 =
* Fixed : Front section image size

= 1.0.27 =
* Fixed : Main visual and front section image placeholder object fit(IE11, Edge)

= 1.0.26 =
* Change : Main visual and front section image object fit(IE11, Edge)
* Change : Post navigation font size
* Fixed : Footer align(IE11, Edge)

= 1.0.25 =
* Fixed : Global nav background color(IE11, Edge)

= 1.0.24 =
* Fixed : typo

= 1.0.23 =
* Add : Global nav do_action(nishiki_before_site_nav_inner_search_button, nishiki_after_site_nav_inner_search_button, nishiki_before_site_nav_inner_menu_collapse_button, nishiki_after_site_nav_inner_menu_collapse_button, nishiki_before_site_nav_inner_menu_button, nishiki_after_site_nav_inner_menu_button)
* Change : Read global nav in global-nav.php

= 1.0.22 =
* Fixed : Footer widget archive count style
* Add : Translate(Link Hover Color)

= 1.0.21 =
* Fixed : Gutenberg editor style
* Add : Theme tags(footer-widgets, full-width-template, portfolio, holiday, news, entertainment, blog, education, food-and-drink)

= 1.0.20 =
* Fixed : Home content display at customizer(front page section)

= 1.0.19 =
* Add : Home content display at customizer(front page section)
* Fixed : Don't smooth scroll at global menu

= 1.0.18 =
* Add : Page template(content-full-width)
* Add : Enable post template

= 1.0.17 =
* Add : Gutenberg style

= 1.0.16 =
* Change : Gutenberg embed style

= 1.0.15 =
* Change : Main css style(column,post navigation)

= 1.0.14 =
* Change : Main css style

= 1.0.13 =
* Change : Css media query breakpoint

= 1.0.12 =
* Change : Main css style
* Change : Several minor changes

= 1.0.11 =
* Change : Editor css style and main css style
* Change : Several minor changes

= 1.0.10 =
* Add : Archive page default eye-catching image(Recommended image size 16:9)

= 1.0.9 =
* Add : Widget area(front widget) beta version
* Add : Translate(front widget)
* Add : CSS style about Gutenberg

= 1.0.8 =
* Add : Customizer(footer widget)
* Add : CSS(footer widget)
* Add : Translate(footer widget)
* Change : Several minor changes

= 1.0.7 =
* Add : header do_action(nishiki_before_singular_header,nishiki_after_singular_header,nishiki_before_archive_header,nishiki_after_archive_header,nishiki_before_search_header,nishiki_after_search_header,nishiki_before_404_header,nishiki_after_404_header)

= 1.0.6 =
* Fixed : Global nav & search overlay open and close script
* Add : Gutenberg editor style(beta)

= 1.0.5 =
* Add : Global nav do_action(nishiki_before_site_nav,nishiki_before_site_nav_inner,nishiki_after_site_nav,nishiki_after_site_nav_inner)

= 1.0.4 =
* Add : YouTube embedded CSS(embed-youtube) when active jetpack plugin

= 1.0.3 =
* Change : Several minor changes

= 1.0.2 =
* Add : "noscroll" class to control smooth scroll of anchor link

= 1.0.1 =
* Add : Support WP 4.9.6
* Fixed : Anchor link smooth scroll

= 1.0.0 =
* Add : Offset anchor to adjust for header
* Change : Several minor changes

= 0.1.40 =
* Remove : Customizer setting_site_display_title(use get_header_textcolor instead of setting_site_display_title)
* Change : Display Site Title and Tagline rule.

= 0.1.39 =
* Add : Page template file(Left Sidebar,Right Sidebar,Bottom Sidebar,No Sidebar)
* Change : Several minor changes

= 0.1.38 =
* Add : Customizer single post author display
* Change : Several minor changes

= 0.1.37 =
* Add : Support WP 4.9.5
* Add : Featured Items
* Add : Apply filters(nishiki_section_num,nishiki_featured_item)
* Add : Nishiki About Page
* Change : Several minor changes

= 0.1.36 =
* Change : Screenshot image(screenshot.png)
* Change : Several minor changes

= 0.1.35 =
* Fixed : Non-printable characters
* Change : Theme URI(https://support.animagate.com/product/wp-nishiki/)

= 0.1.34 =
* Fixed : Non-printable characters

= 0.1.33 =
* Fixed : Custom header image & custom header video
* Change : Several minor changes

= 0.1.32 =
* Add : Single post author description do_action(nishiki_before_single_author_description,nishiki_after_single_author_description)
* Add : Customizer single post author name link
* Change : Several minor changes

= 0.1.31 =
* Fixed : Main visual description

= 0.1.30 =
* Fixed : If placeholder image are not exist, image read twice(main.js)
* Change : Template files(front-page.php,index.php,main_visual.php,section.php)

= 0.1.29 =
* Add : Image Placeholder(Main visual and front page section)
* Add : Setting post thumbnail size(640×360px)
* Change : Setting header video size(640×360px)
* Change : Several minor changes

= 0.1.28 =
* Change : Archive post image size 'large' to 'thumbnail'

= 0.1.27 =
* Fixed : Archive post image size
* Change : Several minor changes

= 0.1.26 =
* Change : Adjust CSS between letters and lines

= 0.1.25 =
* Add : Editor style(editor-style.css).
* Change : Adjust CSS between letters and lines

= 0.1.24 =
* Change : Adjust CSS between letters and lines

= 0.1.23 =
* Add : Support WP 4.9.4
* Add : Responsive Menu(Header Menu Collapse)
* Change : Custom Header Video(header_video_settings)
* Change : Custom Logo size(240x80px)
* Change: Some class and id names
* Change : Several minor changes

= 0.1.22 =
* Add : Support WP 4.9.2
* Change: do_action position(nishiki_before_singular_content,nishiki_after_singular_content)

= 0.1.21 =
* Fixed: Lazy Load(https://wordpress.org/plugins/lazy-load/) plugin Conflict
* Remove : Header title filter(pre_get_document_title)

= 0.1.20 =
* Fixed: Archive page articles CSS

= 0.1.19 =
* Add : Default image of front page setting section (sky.jpg)
* Change : Customizer front page section image color opacity
* Change : Compress image file(carp.jpg,screenshot.png)
* Change : Customizer controll style(customize-controls.js)
* Change : Font-family CSS universal selector to several elements(body,button,input,select,textarea)

= 0.1.18 =
* Add : Image Lazyload(main visual,front_page)
* Add : Translate(Color above the image)
* Change : main CSS(mask)
* Change : Customizer add_control "transport" setting from postMessage to refresh

= 0.1.17 =
* Remove : Customizer CSS(comment area input[type="text"] [type="url"] [type="email"])
* Add : Customizer CSS(input[type="submit"] [type="submit"][disabled])
* Add : Input CSS(input[type="tel"] input[type="date"])
* Add : Theme Description(style.css)

= 0.1.16 =
* Add : Theme Description

= 0.1.15 =
* Add : Support WP 4.9.1
* Add : do_action several pages and parts(header,footer,front_page,singular,archive,search,404)

= 0.1.14 =
* Fixed: Escape

= 0.1.13 =
* Remove : image_send_to_editor
* Remove : set_query_var( 'wp_customize', $wp_customize )
* Change : Non-template PHP files used get_template_directory() instead of get_template_part()
* Change : Several minor changes

= 0.1.12 =
* Add : Pingback URL
* Change : Table CSS(scrollbar)
* Change : add_theme_support called inside of after_setup_theme

= 0.1.11 =
* Fixed: Sanitize
* Fixed: Escape
* Fixed: Customizer "Display Site Title and Tagline"
* Change: Display main visual button
* Remove: Editor style(add it in the future version)
* Remove: Trimming thumbnail image & medium image

= 0.1.10 =
* Change: Customizer CSS

= 0.1.9 =
* Add : Support WP 4.9
* Add : CSS button style
* Add : Several icon font pattern(Icomoon)
* Remove : Customizer add_theme_support "custom-background"
* Change : Customizer add_theme_support "custom-logo"
* Change : Several minor changes

= 0.1.8 =
* Add : Input type=radio style
* Change : Archive category block
* Change : Script and style handle name

= 0.1.7 =
* Fixed : Bug fix

= 0.1.6 =
* Fixed : Bug fix

= 0.1.5 =
* Change : Icon font and more

= 0.1.4 =
* Change : Escape and more

= 0.1.3 =
* Fixed : Archive page column CSS

= 0.1.2 =
* Change : Theme URI style.css

= 0.1.1 =
* Fixed : has_header_video() undefined error in WordPress 4.5
* Add : Theme copyright/license attribute readme.txt
* Change : Date format
* Remove : add_nav_menu.php
* Change : Prefix all the function names
* Add : Support WP 4.5+

= 0.1.0 =
* Initial release
